...
def proj_csimplex(z: np.ndarray, a: float, lower: float = 0.0, upper: float = 1.0) -> np.ndarray:
    """
    Project each row of ``z`` onto a capped simplex in place.

    The projection solves ``argmin_x ||x - z||_2`` subject to
    ``lower <= x_i <= upper`` and ``sum_i x_i = a``.
    """
    u = np.asarray(z)
    ...
    n = u.shape[1]
    if not lower * n <= a <= upper * n:
        raise ValueError("Target sum is infeasible for the given bounds.")

    # The solution has the form clip(z + lambda, lower, upper), so we can
    # recover lambda with a monotone bisection search for each row.
    for row in u:
        lo = np.min(lower - row)
        hi = np.max(upper - row)
